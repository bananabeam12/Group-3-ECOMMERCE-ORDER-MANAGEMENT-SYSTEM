...
redirectIfAuthenticated('profile.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($response['status'])) {
        setFlashMessage('auth_success', 'Account created successfully. You can now sign in.');
    ...
    if (!isset($errors['general']) && !empty($response['message'])) {
        $errors['general'] = $response['message'];
    }
}

$accountLink = isAuthenticated() ? 'profile.php' : 'login.php';
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <nav id="navbar" class="fixed w-full z-50 top-0 start-0 transition-all duration-500 ease-in-out py-6 text-black bg-white shadow-sm">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-6 md:px-10">
            ...
            <div id="nav-icons" class="flex items-center space-x-6 text-sm transition-colors duration-500">
                ...
                <a href="<?php echo htmlspecialchars($accountLink); ?>" class="hover:opacity-60 transition-opacity">
        ...
        <div class="w-full max-w-4xl px-6 text-center">
            ...
            <form action="signup.php" method="POST" class="space-y-8">
                ...
                <div class="flex flex-col items-center gap-4 pt-4">
                    ...
                    <a href="login.php" class="text-sm font-medium hover:underline">Cancel</a>
                </div>
            </form>

            <div class="mt-8 space-y-2">
                <div>
                    <a href="login.php" class="text-sm font-medium hover:underline">Back to Login</a>
                </div>
                <div>
